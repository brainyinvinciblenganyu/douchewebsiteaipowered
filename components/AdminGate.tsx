...
import { Loader2, LogOut, ShieldCheck, Copy } from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { href: '/ops-7f3k2/inbox', label: 'Inbox' },
  { href: '/ops-7f3k2/audit-log', label: 'Audit Log' },
];

type Stage = 'checking' | 'credentials' | 'totp' | 'setup' | 'recoveryCodes' | 'loggedIn';
...
export default function AdminGate({ children }: { children: ReactNode }) {
  ...
  const submitCredentials = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/login', {
        ...
        body: JSON.stringify({ email, password, code: stage === 'totp' ? code : undefined }),
      });
  ...
  if (stage === 'credentials' || stage === 'totp') {
    return (
      <main className="mx-auto flex min-h-[70vh] max-w-md items-center justify-center px-6">
        ...
        >
          ...
          <h1 className="mt-3 text-2xl font-bold text-[#111827]">
            {stage === 'totp' ? 'Enter your authenticator code' : 'Admin sign in'}
          </h1>
          ...
          <button
            type="submit"
            disabled={submitting || (stage === 'credentials' ? !email || !password : !code)}
          ...
          >
            ...
            {submitting ? 'Checking...' : stage === 'totp' ? 'Verify' : 'Continue'}
          </button>
        </form>
      </main>
    );
    ...
    return (
      <main className="mx-auto flex min-h-[70vh] max-w-md items-center justify-center px-6">
        ...
        >
          ...
          <h1 className="mt-3 text-2xl font-bold text-[#111827]">Set up two-factor authentication</h1>
          <p className="mt-2 text-sm text-slate-600">
          ...
          {qrDataUri && (
            ...
            <img src={qrDataUri} alt="TOTP QR code" className="mx-auto mt-5 h-48 w-48 rounded-lg border border-[#D1D5DB]" />
          )}
          ...
          <input
            ...
            className="mt-5 w-full rounded-lg border border-[#D1D5DB] bg-white px-4 py-3 text-[#111827] outline-none transition focus:border-transparent focus:ring-2 focus:ring-[#0058A3]"
          />
