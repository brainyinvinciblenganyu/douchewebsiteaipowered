...
import { type DbProduct } from '../../../../lib/db/queries.js';

// A real catalog product plus its real average rating (from product_reviews),
// used throughout scoring in place of the old mock Product type.
export type ScorableProduct = DbProduct & { rating: number };
...
export function scoreSearchMatch(product: ScorableProduct, recentSearches: string[]): number {
...
export function scoreCategoryMatch(product: ScorableProduct, favoriteCategories: ShoppingProfile['favoriteCategories']): number {
  if (!favoriteCategories.length || !product.category) return 0;
...
export function scorePurchaseHistory(product: ScorableProduct, purchasedIds: string[], allProducts: ScorableProduct[]): number {
...
export function scoreRecentlyViewed(product: ScorableProduct, recentlyViewedIds: string[], allProducts: ScorableProduct[]): number {
  ...
  if (product.category && recentCategories.has(product.category)) {
    return 10;
  }
...
export function scoreWishlist(product: ScorableProduct, wishlistIds: string[]): number {
...
export function scoreCartCompatibility(product: ScorableProduct, cartIds: string[], allProducts: ScorableProduct[]): number {
  ...
  for (const id of cartIds) {
    ...
    if (cartProd?.category && product.category) {
...
export function scoreBrandPreference(product: ScorableProduct, favoriteBrands: ShoppingProfile['favoriteBrands']): number {
  if (!favoriteBrands.length || !product.vendor_user_id) return 0;

  // Top preferred vendor gets 15 points
  if (product.vendor_user_id === favoriteBrands[0].brand) {
    return 15;
  }

  const inHistory = favoriteBrands.some((fb) => fb.brand === product.vendor_user_id);
  return inHistory ? 5 : 0;
}

export function scoreBudgetMatch(product: ScorableProduct, range: ShoppingProfile['preferredPriceRange'], avgSpending: number): number {
  ...
  if (avgSpending > 0) {
    const diffPercent = Math.abs(price - avgSpending) / avgSpending;
    if (diffPercent <= 0.3) {
      return 8;
    }
  }
...
export function scoreTrending(product: ScorableProduct, trendingCounts: { productId: string; count: number }[]): number {
  let points = 0;

  // Add points based on real average rating (1-5 range)
